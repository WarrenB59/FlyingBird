...
const gravity = 0.5; // gravité assignée à l'oiseau
...
let index = 0, // vitesse du fond != de la vitesse des poteaux (const speed) effet parallax
  bestScore = 0,
  currentScore = 0,
  pipes = [], // tableaux pour faire apparaître les poteaux de façon aléatoires
  flight, // vol
...
const render = () => {
  ...
  if (gamePlaying) {
    ...
    ctx.fillText(`Meilleur score : ${bestScore}`, 55, 245); // écrire sur un canva .fillText blablabla, abscisse, ordonnée
  ...
  if (gamePlaying) {
    pipes.map((pipe) => {
      ...
      if (pipe[0] <= -pipeWidth) {
        ...
        bestScore = Math.max(bestScore, currentScore); // vérifie le "bestScore" != de currentScore

        pipes = [
          ...pipes.slice(1),
          [pipes[pipes.length - 1][0] + pipeGap + pipeWidth, pipeLoc()],
        ]; // spread operator du [] ...pipes le .slice pour enlever un élément du [] puis en ajouter un un
      }
      // si bird touche un poteau fin de partie  .every pour verifier toutes les conditions d'un[] cf doc google

      if (
        [
          pipe[0] <= cTenth + size[0], // vérifie que bird soit bien sur l'axe des y true
          pipe[0] + pipeWidth >= cTenth, // vérifie que bird soit bien sur l'axe des y true
